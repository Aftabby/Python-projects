#Data Structure
# 1. Strore mkore than one data
# 2. Special Structure to srore data
# 3. Specific rule to add or remove data
# Examples: List, Dictionary, Linked List
# If follows, LIFO policy - is called STACK, else if follows FIFO policy - is called QUEUE.
# Dictionary type data structure contains key:value pair
# Linked list - node use pointers to link with each other
# Tree - When a data structure starts with a root and each node can have one or more children
...
# Similarly, any node connected on the upward is called parent node of the node under it
...
best_duo["Tahsan"] = "Mithila"   #Adding element in dictionary
...
del best_duo[34]  # delete element in dictionary
print(best_duo)
...
all_players = list(all_players)       # Converting all the keys of dictionary into a list
print(all_players, type(all_players), "\n\n")


all_ratings = player_rating.values() # Similarly, to get all the values of a dictionary use -- .values() -- method
print(all_ratings, type(all_ratings))

all_ratings = list(all_ratings)
print(all_ratings, "\n\n") 

for key_name in player_rating:
    print(key_name, type(key_name))   # here the loop_variable is key
...
queue.pop(0)   #First in First out - FIFO
...
# If a queue is  designed such a way to give higher priority to a special type of elements, that queue is called priority queue
# A dictionary is like a couple, to declare a dictionary -- dictionary_name = {"Key" : "Value", "key1" : "value2"}   ------- key and value can be of any data type (numeric, string, boolean)
# To access a dictionary element's Value ---  print(dictionary_name["Key"]) ----
# To access an item in list we need to know index, but for dictionary we need to know index. Actual index position doesn't matter in dictionary
...
# Similarly, to get all the values of a dictionary use -- .values() -- method
