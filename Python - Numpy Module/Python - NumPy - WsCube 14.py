...
arr3 = np.insert(arr1, (0, 3), 90)      #If you want to enter the same value to multiple index position, you can do that as well
print(arr3)                             

arr4 = np.insert(arr1, 2, 99.56)        # Also keep in mind, if you wanna insert a float value, to an integer array, it will only take the integer part of that element, leaving the part after the decimal
print(arr4)
...
arr6 = np.insert(arr5, 2, 60, axis=0)       # np.insert(array, index_position, value, axis=0,1..) , values and index_position can be multiple - pass it as a tuple
print(arr6)

arr7 = np.insert(arr5, 2, (60, 70, 80), axis=0)       
print(arr7)
