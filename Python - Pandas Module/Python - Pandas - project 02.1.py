...
job1 = df1["JobTitle"].str.contains("captain", case=False)      #By default, case=True (case sensitive)
job2 = len(df1)             #either use len() function or also you can use count() method on it
...
fillna1 = df1["EmployeeName"].fillna("Not Provided")    
print(fillna1)

fillna3 = df1["EmployeeName"].replace(np.nan, "Not provided")   #It also works same as the -- .fillna -- method , as there is np.nan as first parameter
print(fillna3)
...
drop5 = df1.drop(drop4.index, axis=0)       #Dropping the row with -- drop() -- method takes rows_index as first parameter,, also by default the axis=0(i.e row wise)
...
job3 = df1["EmployeeName"].str.upper()  == str.upper("Albert Pardini")  #We don't know the uppercase or lowercase of name exist in dataset, so we uppercased both side string to check
