...
range2 = dframe1[range1]        #You already know if we pass a list of boolean values(which has elements equal to dataframe's row number) as it's index position, we get only the rows that was True in the list,, guess what? same things happen for also the  series_object not only list 
print(range2)
...
range5 = dframe1["Marks"].between(90, 100)          #In -- between -- method both the values are inclusive,, it works same like what we assigned in the variable -- range3 --
print(range5)
...
print("\nUse map function to a column")
map1 = dframe1["Gender"].map({'Male' : 1, "Female" : 0})    #In -- series_object.map() -- method, you pass a dictionary, If any -- key -- of that dictionary matches any value of the series, the value of the series gets replaced by the value of that particular dictionary -- key -- ,, and returns a new series with replaced value 
...
column4 = column3[["Name", "Marks"]]        #Here column name is in 2D list because if we pass more than one column value, it will be takes as more than one parameter
print(column4)
